...
class ImageMetaDataset(Dataset):
    ...
    def input_name(self):
        if not self.__input_name:
            raise ValueError(
                "The input is not yet defined, so __input_name is not defined."
            )
    ...
    def target_name(self):
        if not self.__target_name:
            raise ValueError(
                "The target is not yet defined, so __target_name is not defined."
            )
        return self.__target_name

    def __getitem__(self, _idx: int):
        """Returns DAPI and GOLD image stains and names."""

        self.__input_name = self.__image_path[_idx].name
        self.__target_name = (
            str(self.__input_name).replace("CH0", "CH2").replace("dapi", "gold")
        )

        # Convert the images to 16-bit arrays
        input_image = np.array(
            Image.open(self.__input_dir / self.__input_name).convert("I;16")
        )
        target_image = np.array(
            Image.open(self.__target_dir / self.__target_name).convert("I;16")
        )
        ...
        return (
            input_image,
            target_image,
            {"input_name": self.__input_name, "target_name": self.__target_name},
        )
